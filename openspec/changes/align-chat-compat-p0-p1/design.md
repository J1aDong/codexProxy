...
### 5.1 Phase 1 对 thinking 完整性的边界澄清
- **决策**：Phase 1 仅保证 `thinking_delta` 与 `stop_sequence` 的下游行为；`signature_delta` 暂不在通用 OpenAI Chat Completions 路径中实现。
...
- **How to apply**：通用 `openai.rs` backend 不引入私有 signature 字段猜测；如需支持，必须以下游/上游对明确、稳定、可验证的 provider-specific 字段为前提。
