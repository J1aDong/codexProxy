...
- CI Linux 构建基线固定为 `ubuntu-22.04`，避免 `ubuntu-latest` 漂移带来的 glibc 兼容问题
...
- Ubuntu 用户优先使用 `DEB` 包（依赖通常会自动拉取）
- `AppImage` 在部分系统上需要手动安装上述依赖
...
### 4. Ubuntu 提示缺少 `libwebkit2gtk-4.1-0`

这是 Linux 桌面运行时依赖缺失导致。执行“Linux 运行时依赖（Ubuntu）”章节中的安装命令后再启动。

### 5.  当使用codex5.3等模型 文本和工具调用混合乱码时
可以在claude code中输入
`
返回的数据文本和工具调用不要混在一起。继续操作。
` 或者可以把此文本做成自定义命令方便调用。 codex api在claude code调用时由于和codex cli不是完全匹配的调用方式，受系统提示词的限制等条件，上游很容易出现文本+工具调用的溢出文本，在本转换工具中很难处理此异常行为，可以手动提示词干预的方式规范上游的行为。
